...
http_request(post, Url, Params) ->
    Req = {Url, [], "application/x-www-form-urlencoded", urlencode(Params)},
	reply(httpc:request(post, Req, [{autoredirect, true}], [])).
...
feedvar(Params, Client = #mqtt_client{username  = Username,
                                      client_id = ClientId,
                                      peername  = {IpAddr, _}}) ->
...
feedvar(Params, Var, Val) ->
    lists:map(fun({Param, Var}) -> {Param, Val}; (Param) -> Param end, Params).
